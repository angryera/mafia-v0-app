...
export function BoozeAction() {
  ...
  const fetchHoldings = useCallback(async () => {
    ...
    try {
      ...
      while (hasMore) {
        ...
        // Filter for booze (categoryId === 0) and not sold
        const boozeGoods = goods
          .filter((good) => Number(good.categoryId) === 0 && !good.isSold)
          .map((good) => ({
            id: Number(good.id),
            owner: good.owner,
  ...
  const fetchPrices = useCallback(async () => {
    ...
    try {
      ...
      const boozePrices = result[0].map((p) => Number(formatEther(p)));
      setPrices(boozePrices);
    } catch (error) {
  ...
  useEffect(() => {
    if (limitsData) {
      setBoozeLimit(Number((limitsData as [bigint, bigint])[0]));
    }
  }, [limitsData]);
      ...
      try {
        ...
        if (decoded.eventName === "BoozeBuy") {
          const args = decoded.args as {
            ...
            isSuccess: boolean;
            isJailed: boolean;
            totalUserBoozeCount: number;
            cashAmount: bigint;
            xpPoint: bigint;
            nextBoozeTime: bigint;
            timestamp: bigint;
          };
          // Determine result status: Success, Jailed, or Failed
          // isSuccess -> Success
          // isJailed -> Jailed
          // !isSuccess && !isJailed -> Failed
          ...
          return {
            ...
            totalUserBoozeCount: Number(args.totalUserBoozeCount),
  ...
  useEffect(() => {
    ...
    if (buyResult.status === "success") {
      toast.success(`Successfully bought booze! You now have ${buyResult.totalUserBoozeCount} items. Cost: $${buyResult.cashAmount.toLocaleString()}`);
  ...
  useEffect(() => {
    ...
    if (sellResult.status === "success") {
      toast.success(`Successfully sold booze for $${sellResult.cashAmount.toLocaleString()}! You now have ${sellResult.totalUserBoozeCount} items.`);
  ...
  const setBuyAmount = (typeId: number, amount: number) => {
    const maxAllowed = boozeLimit - totalHoldings;
  ...
  const handleBuy = async () => {
    ...
    try {
      ...
      toast.error("Failed to buy booze");
    }
  };
  ...
  const handleSell = async () => {
    ...
    try {
      ...
      toast.error("Failed to sell booze");
    }
  };
      ...
      <div className="rounded-xl border border-border bg-card p-6">
        <div className="flex flex-col items-center justify-center py-12 text-center">
          ...
          <p className="text-sm text-muted-foreground max-w-sm">
            Connect your wallet to access the Booze Warehouse
          </p>
        </div>
      </div>
    );
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className="font-medium text-foreground">{totalHoldings} / {boozeLimit}</span>
            </div>
          </div>
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 text-sm">
      ...
      <div className="rounded-xl border border-border bg-card overflow-hidden">
        <table className="w-full">
          ...
          <tbody>
            {Object.entries(BOOZE_TYPES).map(([typeIdStr, name]) => {
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-center gap-2">
                      ...
                      <input
                        type="number"
                        min={0}
                        max={mode === "buy" ? boozeLimit - totalHoldings : holdingCount}
                        value={amount}
                        onChange={(e) => {
                      ...
                      <button
                        ...
                        disabled={
                          mode === "buy"
                            ? totalBuyAmount >= boozeLimit - totalHoldings
                            : amount >= holdingCount
                        }
                        className="rounded p-1 hover:bg-muted disabled:opacity-30"
                      >
