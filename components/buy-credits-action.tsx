...
// Represents a swap token returned by the contract
type SwapToken = {
  name: string;
  tokenAddress: `0x${string}`;
  isStable: boolean;
  isEnabled: boolean;
  price: bigint; // raw price from contract (wei)
  decimal: number;
  tokenId: number;
  formattedPrice: number; // price in human-readable form
};

export function BuyCreditsAction() {
  ...
  const { data: swapData, error: swapError, isLoading: swapLoading } = useReadContract({
    ...
    abi: BUY_CREDIT_CONTRACT_ABI,
    functionName: "getSwapTokens",
    chainId: chainConfig.wagmiChainId,
  });

  // Fetch helper credit price from the contract
  const { data: creditPriceRaw, error: priceError } = useReadContract({
    address: addresses.buyCredit,
    abi: BUY_CREDIT_CONTRACT_ABI,
    functionName: "price",
    chainId: chainConfig.wagmiChainId,
  ...
  const swapTokens: SwapToken[] = useMemo(() => {
    ...
    // viem returns multiple outputs as a readonly tuple [structArray, pricesArray]
  ...
  // Calculate the payment amount in the selected token
  const paymentAmount = useMemo(() => {
    if (!selectedToken || !creditPriceUsd) return null;
    if (selectedToken.formattedPrice <= 0) return null;

    // total USD cost / token price in USD = tokens needed
    const totalUsd = amount * creditPriceUsd;
    const tokensNeeded = totalUsd / selectedToken.formattedPrice;
    return tokensNeeded;
  }, [selectedToken, creditPriceUsd, amount]);

  // Wei value for the contract call
  ...
  // For native token (tokenId 0), we need to send value with 0.1% buffer
  ...
  // Check ERC20 allowance (only for non-native tokens)
  ...
  const { data: currentAllowance, refetch: refetchAllowance } =
    useReadContract({
      ...
      args: address ? [address, addresses.buyCredit] : undefined,
      chainId: chainConfig.wagmiChainId,
  ...
  // Auto-advance to buy step if allowance is sufficient
  useEffect(() => {
  ...
  const handleApprove = () => {
    ...
    writeApprove({
      ...
      args: [addresses.buyCredit, paymentAmountWei],
    });
  };

  const handleBuy = () => {
    if (!selectedToken || amount < 1 || paymentAmountWei === BigInt(0)) return;
    resetBuy();
  ...
  const totalCostUsd = creditPriceUsd ? amount * creditPriceUsd : null;
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="rounded-md bg-background/50 px-3 py-2">
            ...
            <div className="mt-1.5 flex items-center justify-between">
              <span className="text-xs text-muted-foreground">amount (wei)</span>
          ...
          {/* Action buttons */}
          {step === "approve" && !hasEnoughAllowance && !isNativeToken ? (
            <button
            ...
            >
              {isApproveLoading ? (
                ...
                <>
                  ...
                  {isConnected
                    ? `Approve ${selectedToken?.name ?? "token"}`
            ...
            <button
              onClick={handleBuy}
              disabled={!isConnected || isBuyLoading || amount < 1 || !paymentAmount}
              className={cn(
                "flex w-full items-center justify-center gap-2 rounded-lg px-4 py-3 text-sm font-semibold transition-all duration-200",
                isConnected
                  ? "bg-primary text-primary-foreground hover:brightness-110 active:scale-[0.98] disabled:opacity-50"
            ...
            >
              {isBuyLoading ? (
                ...
                <>
                  ...
                  {isConnected
                    ? `Buy ${amount} Credit${amount !== 1 ? "s" : ""}${totalCostUsd !== null ? ` - $${totalCostUsd.toFixed(2)}` : ""}`
                    : "Connect Wallet First"}
                </>
              )}
            </button>
