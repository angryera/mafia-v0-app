...
export function BuyKeysAction() {
  ...
  // Fetch crate price from the contract
  const { data: cratePriceRaw } = useReadContract({
    address: addresses.buyKeys,
    abi: BUY_KEYS_CONTRACT_ABI,
    functionName: "price",
    chainId: chainConfig.wagmiChainId,
  ...
  const cratePriceUsd = cratePriceRaw
    ? Number(formatUnits(cratePriceRaw as bigint, 18))
    : null;
  ...
  const paymentAmount = useMemo(() => {
    if (!selectedToken || !cratePriceUsd) return null;
    if (selectedToken.formattedPrice <= 0) return null;
    const totalUsd = amount * cratePriceUsd;
    const tokensNeeded = totalUsd / selectedToken.formattedPrice;
    return tokensNeeded;
  }, [selectedToken, cratePriceUsd, amount]);
  ...
  const { data: currentAllowance, refetch: refetchAllowance } =
    useReadContract({
      ...
      args: address ? [address, addresses.buyKeys] : undefined,
      chainId: chainConfig.wagmiChainId,
  ...
  const handleApprove = () => {
    ...
    writeApprove({
      ...
      args: [addresses.buyKeys, paymentAmountWei],
    });
  };
  ...
  const totalCostUsd = cratePriceUsd ? amount * cratePriceUsd : null;

  return (
    <div>
      <div className="mb-6">
        <h2 className="text-lg font-bold text-foreground">Buy Key Crates</h2>
        <p className="mt-0.5 text-sm text-muted-foreground">
          Select a payment token and purchase key crates via{" "}
        ...
        <div className="flex flex-col gap-6">
                ...
                {totalCostUsd !== null ? (
                  <>
                    ...
                    <p className="text-[11px] text-muted-foreground">
                      {amount} crate{amount !== 1 ? "s" : ""} x $
                      {cratePriceUsd?.toFixed(2)}
                    </p>
                  </>
                ) : (
          ...
          <div className="rounded-md bg-background/50 px-3 py-2">
            ...
            <div className="mt-1.5 flex items-center justify-between">
              <span className="text-xs text-muted-foreground">crateAmount</span>
              <span className="font-mono text-xs text-foreground">
                {amount}
              </span>
            </div>
            ...
            >
              {isBuyLoading ? (
                ...
                <>
                  ...
                  {isConnected
                    ? `Buy ${amount} Crate${amount !== 1 ? "s" : ""}${totalCostUsd !== null ? ` - $${totalCostUsd.toFixed(2)}` : ""}`
                    : "Connect Wallet First"}
                </>
              )}
            </button>
